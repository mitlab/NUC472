 ...
 * @version  V1.00
 * $Revision: 3 $
 * $Date: 15/11/18 10:55a $
 * @brief    Demonstrate how to trigger EADC by STADC external signal.
 *
 * @note
...
void SYS_Init(void)
{
    ...
    /* Configure STADC pins (GPD2) */
    SYS->GPD_MFPL = (SYS->GPD_MFPL & ~SYS_GPD_MFPL_PD2MFP_Msk) | SYS_GPD_MFPL_PD2MFP_STADC;
...
void EADC_FunctionTest()
{
    ...
    printf("\n");
    printf("+----------------------------------------------------------------------+\n");
    printf("|                      External Pin (STADC) trigger mode test          |\n");
    printf("+----------------------------------------------------------------------+\n");

    printf("\nIt demonstrates external pin (STADC) trigger function.\n");
    printf("Please connect PD.2 with PD.3\n\n");

    /* Enable the A/D converter */
    EADC_Open(EADC, 0);

    /* Configure the EADC0 sample module 0 for analog input channel 2 and external pin(STADC) trigger source.*/
    EADC_ConfigSampleModule(EADC, EADC0_SAMPLE_MODULE0, EADC_STADC_TRIGGER, EADC0_CH2);
