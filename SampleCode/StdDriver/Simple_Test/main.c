...
void SYS_Init(void)
{
	...
	/* Set PB multi-function pins for UART1 RXD, TXD */
   SYS->GPF_MFPH &= ~(SYS_GPF_MFPH_PF14MFP_Msk | SYS_GPF_MFPH_PF13MFP_Msk);
...
#else
		/* Set PB multi-function pins for UART2 RXD, TXD */
    SYS->GPH_MFPH &= ~(SYS_GPH_MFPH_PH11MFP_Msk | SYS_GPH_MFPH_PH12MFP_Msk);
    SYS->GPH_MFPH |= (SYS_GPH_MFPH_PH11MFP_UART3_RXD | SYS_GPH_MFPH_PH12MFP_UART3_TXD);
#endif
...
void UART_Init(UART_T *uartport, uint32_t u32baudrate, uint32_t u32intsel, IRQn_Type irqn, uint32_t resetIndex)
{
	/* Reset IP */
	if(resetIndex != NULL) {
		SYS_ResetModule(resetIndex);
	}
...
int main(void)
{
    ...
    UART_Init(UART0, UART0_BAUD_RATE, (UART_INTEN_RDAIEN_Msk | UART_INTEN_RXTOIEN_Msk), UART0_IRQn, UART0_RST);
	UART_Init(UART1, UART1_BAUD_RATE, (UART_INTEN_RDAIEN_Msk | UART_INTEN_RXTOIEN_Msk), UART1_IRQn, UART1_RST);
	UART_Init(UART2, UART2_BAUD_RATE, (UART_INTEN_RDAIEN_Msk | UART_INTEN_RXTOIEN_Msk), UART2_IRQn, UART2_RST);
	UART_Init(UART3, UART3_BAUD_RATE, (UART_INTEN_RDAIEN_Msk | UART_INTEN_RXTOIEN_Msk), UART3_IRQn,	UART3_RST);
    UART_Init(UART4, UART4_BAUD_RATE, (UART_INTEN_RDAIEN_Msk | UART_INTEN_RXTOIEN_Msk), UART4_IRQn, UART4_RST);
	UART_Init(UART5, UART5_BAUD_RATE, (UART_INTEN_RDAIEN_Msk | UART_INTEN_RXTOIEN_Msk), UART5_IRQn, UART5_RST);
