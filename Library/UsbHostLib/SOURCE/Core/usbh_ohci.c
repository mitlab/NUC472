 ...
 * @version  V1.00
 * $Revision: 9 $
 * $Date: 14/10/31 10:30a $
 * @brief    NUC472/NUC442 MCU USB Host OHCI driver
 *
 * @note
...
static int  ep_unlink(ED_T *ed)
{
    ...
    case PIPE_CONTROL:
        if (ed->ed_prev == NULL) {
            ...
            USBH->HcControlHeadED = ed->hwNextED;
        } else
...
static void  ep_rm_ed(USB_DEV_T *usb_dev, ED_T *ed)
{
    ...
    if (!ohci.disabled) {
        ...
        USBH->HcInterruptStatus = OHCI_INTR_SF;
        USBH->HcInterruptEnable = OHCI_INTR_SF;
    }
}
...
static void  iso_td_fill(uint32_t info, void *data, URB_T *urb, int index)
{
    ...
    USB_info("Frame:[%x], CBP:[%x], BE:[%x], PSW0:[%x], PSW1:[%x]\n",
...
static void td_submit_urb(URB_T * urb)
{
    ...
    case PIPE_BULK:
        ...
        cnt++;
        USBH->HcCommandStatus = OHCI_BLF;      /* start bulk list */
        break;
    ...
    while (td_list_hc) {
        ...
        if (TD_CC_GET((uint32_t)td_list->hwINFO)) {
            ...
            if (td_list->ed->type == PIPE_ISOCHRONOUS) {
                ...
                USB_error("CC Error!! - DEV[%d] ED[%d] - ", td_list->urb->dev->devnum, (td_list->ed->hwINFO >> 7) & 0xf);
...
static void  dl_del_list(uint32_t frame)
{
    ...
    if (!ohci.disabled) {
        if (ctrl)       /* reset control list */
            USBH->HcControlCurrentED = 0;
        if (bulk)       /* reset bulk list */
            USBH->HcBulkCurrentED = 0;
...
#define WR_RH_STAT(x)           (USBH->HcRhStatus = (x))
#define WR_RH_PORTSTAT(x)       (USBH->HcRhPortStatus[wIndex-1] = (x))
#define RD_RH_STAT              (USBH->HcRhStatus)
#define RD_RH_PORTSTAT          (USBH->HcRhPortStatus[wIndex-1])
...
static int  rh_submit_urb(URB_T *urb)
{
    ...
    if (usb_pipeint(pipe)) { /* 如果這個 URB 是屬於 interrupt pipe */
        ohci.rh.urb =  urb;
        ohci.rh.send = 1;
    ...
    */
    ...
    case RH_GET_DESCRIPTOR | RH_CLASS: {
        uint32_t temp = USBH->HcRhDescriptorA;
...
static int  ohci_reset()
{
    ...
    while ((USBH->HcCommandStatus & OHCI_HCR) != 0) {
        if (--timeout == 0) {
    ...
    USBH->HcControl = (USBH->HcControl & 0xffffff3f) | OHCI_USB_RESET;
...
void USBH_IRQHandler(void)
{
    ...
    if (ints & OHCI_INTR_WDH) {
        ...
        USBH->HcInterruptStatus = OHCI_INTR_WDH;
    }
    ...
    if (ints & OHCI_INTR_SO) {
        ...
        USBH->HcInterruptDisable = OHCI_INTR_SO;
        USBH->HcInterruptStatus = OHCI_INTR_SO;
    }

    if (ints & OHCI_INTR_SF) {
        uint32_t frame = ohci.hcca->frame_no & 1;

        USBH->HcInterruptDisable = OHCI_INTR_SF;
    ...
    if (ints & OHCI_INTR_RHSC) {
        USBH->HcInterruptDisable = OHCI_INTR_RHSC;
        USBH->HcInterruptStatus = OHCI_INTR_RHSC;
    }

    if (ints & OHCI_INTR_RD) {
        USBH->HcInterruptDisable = OHCI_INTR_RD;
        USBH->HcInterruptStatus = OHCI_INTR_RD;
    }

    ints &= ~OHCI_INTR_WDH;
    USBH->HcInterruptEnable = OHCI_INTR_MIE;
...
static int  hc_start ()
{
    ...
    USBH->HcHCCA = (uint32_t)ohci.hcca; /* a reset clears this */
    ...
    USBH->HcRhDescriptorA = (USBH->HcRhDescriptorA | RH_A_NPS) & ~RH_A_PSM;
    USBH->HcRhStatus = RH_HS_LPSC;

    /* POTPGT delay is bits 24-31, in 2 ms units. */
    usbh_mdelay((USBH->HcRhDescriptorA >> 23) & 0x1fe);
